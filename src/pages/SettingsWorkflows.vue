...
div.row.q-mb-md
  // page title
  div.column
    span.text-h6 Manage your Workflows
    span.text-caption.text-grey-6 Create or manager your workflows
...
q-table(
  ...
  rows-per-page-label="Workflows per page"
  ...
  template(#no-data)
    div.fit.column.items-center.q-py-xl
      ...
      div.text-subtitle2.q-mb-xs
        | This is where you'll manage your workflows
...
import { fetchWorkflows, TRIGGERS, TRIGGERS_MAP } from 'boot/workflows';
...
export default {
  ...
  setup () {
    // table
    const dataset = fetchWorkflows({ fetchImmediate: true });
    ...
    const columns = [
      ...
      },
      {
        name: 'trigger',
        field: 'trigger',
        format: val => TRIGGERS_MAP[val.event]?.name || val.event || '',
        label: 'Trigger',
        align: 'left',
        sortable: false,
        style: 'width: 120px',
    ...
    const onCreate = handleAction(async (data) => {
      ...
      loaderMessage: 'Creating workflow...',
    });
    const onToggleActive = handleAction(dataset.toggleItemActiveStatus, {
      errorPrefix: 'Failed to set workflow active status',
      loaderMessage: item => {
      ...
      confirmMessage: item => {
        ...
        return `Would you like to ${operation} ${item.label} workflow?`;
      },
    });
    const onDelete = handleAction(dataset.deleteItem, {
      errorPrefix: 'Failed to delete workflow',
      loaderMessage: item => `Deleting ${item.label} workflow...`,
      confirmMessage: item => `Would you like to delete ${item.label} workflow? This action cannot be reversed`,
    });
